...
class _Dumper:
    ...
    def dump_specs(self, specs: list[Spec]) -> None:
        for spec in specs:
            ...
            if spec.namespace == GLOBAL:
                self._builtins_ref = _BUILTINS_YAML
            else:
                self._builtins_ref = "../" + _BUILTINS_YAML
    ...
    def _dump_resp(
        ...
        if result is None:
            schema["$ref"] = (
                self._builtins_ref + "#/components/schemas/rpcRespWithoutResult"
            )
            return
        ...
        properties = {
            ...
            "error": {
                "$ref": self._builtins_ref + "#/components/schemas/rpcError",
    ...
    def _dump_field(self, field: Field, property: dict) -> None:
        ...
        if field_type.is_repeated:
            ...
            property2 = property
        if field_type.is_model_ref:
            if field_type.namespace is None:
                namespace = self._namespace
            else:
                namespace = field_type.namespace
            if namespace == self._namespace:
                if self._file_name == _MODELS_YAML:
                    models_ref = ""
                else:
                    models_ref = _MODELS_YAML
            else:
                if namespace == GLOBAL:
                    models_ref = "../" + _MODELS_YAML
                else:
            ...
            property2["$ref"] = (
                models_ref
                + "#/components/schemas/"
                + utils.camel_case(field_type.value)
            )
        else:
            property2.update(
                {
